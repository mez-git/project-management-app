import React, { useEffect, useState } from 'react';
    ...
    function UserManagementPage() {
      ...
      const [userFormData, setUserFormData] = useState({ name: '', email: '', password: '', role: 'Team Member' });
      ...
      const handleSaveUser = async () => {
        try {
          ...
          setError(err.response?.data?.error || `Failed to ${currentUser ? 'update' : 'create'} user.`);
        }
      };
                  ...
                  {users.map((userItem) => (
                    <TableRow key={userItem._id}>
                      ...
                      <TableCell align="right">
                        <IconButton onClick={() => handleOpenEditUser(userItem)} color="secondary">
                          <EditIcon />
                        </IconButton>
                        {userItem._id !== user._id && ( // Prevent admin from deleting themselves
                          <IconButton onClick={() => handleDeleteUser(userItem._id)} color="error">
                            <DeleteIcon />
                          </IconButton>
                        )}
                      </TableCell>
